...
2. **Cite every claim** with a VALID `[Fact N]`/`[Ref N]` (N present in the evidence). A
   self-asserted `[gloss]` tag does NOT count as grounding — the guard cannot tell a real
   paraphrase from a fabricated claim wearing the tag — so `[gloss]` is tracked separately and
   capped (>20% of sentences ⇒ refuse).
...
5. **Refuse, don't annotate.** `refusal_reason()` returns non-None on ANY hard guard — a
   forbidden move, a fabricated citation, a bad anchor, sub-threshold grounding
   (`REPORT_LLM_GROUNDING_MIN`, default 0.8), or excess gloss — and the route then **discards
   the LLM text and serves the deterministic fallback** (the report's own plain prose). The LLM's
   words never reach the user when any guard trips.
