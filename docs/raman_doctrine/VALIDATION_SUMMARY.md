...
| 35 | close the cleanly-definable yoga-coverage gap | **landed (reading engine)** — encoded 8 occupancy-only yogas faithful to Raman's definitions (Dhurdhura, Chatussagara, Vasumathi, Sakata, Chakra, Gola/Yuga/Sula) into `app/core/yoga_library.py`. Coverage **26.6% → 37.5%** (17→24/64). 46 yoga unit tests green; `house_judgment.py`/`synthesis_v2.py` byte-untouched (reading path, no strength-grade impact). Pinned by `test_yoga_coverage` |
