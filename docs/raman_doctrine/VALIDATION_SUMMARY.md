...
| 33 | identity-preserving learned scorer (the ML/representation lever) | **documented negative** — a 50-dim identity representation (per-frame criterion counts, aspect-source nature, tier/besieged/fort), uniform across held-out + NH + the live anchor, strictly nesting A2's aggregates. On the same LOCO folds it LOSES to A2's `(delta,frame)` tokens (best **57.9%** vs **61.4%**), overfits more (gaps +9..+23 vs A2's +3.2), and fails the live anchor gate **0/9** (engine 2/9). The identity signal is real but not learnable at N≈130 — richer representation + tiny data = worse generalization. Closes the ML lever; the a-priori doctrine gates beat both fitted models on the anchor. `identity_scorer.py`, pinned by `test_identity_scorer` |
| 34 | Three Hundred Combinations — fresh yoga corpus + coverage audit (new axis) | **fresh data + coverage finding** — the strength vein is exhausted across ALL Raman's books (archive.org catalog check: only HTJAH + NH carry graded strength verdicts, both mined). Pivoted to yoga-outcome data: extracted 64 yogas + 39 example charts (`three_hundred_yogas.json`). Coverage audit: the engine's yoga library (29 detectors) covers **17/64 = 26.6%** of Raman's named yogas — it knows the major yogas but is blind to the long tail (Nabhasa, Chatussagara, Sakata, dhana yogas), the mechanical reason the incr-30 yoga token was weak. Increment 35 closes the cleanly-definable part. Outcome prediction NOT scored (doctrine → circular; needs population data). `yoga_coverage.py`, pinned by `test_yoga_coverage` |
| 35–36 | close the cleanly-definable yoga-coverage gap | **landed (reading engine)** — encoded 15 yogas faithful to Raman's definitions: 8 occupancy-only (Dhurdhura, Chatussagara, Vasumathi, Sakata, Chakra, Gola/Yuga/Sula) + 7 lord-based from a cleaner OCR edition (Parvata, Kahala, Chapa, Sreenatha, Sankha, Bheri, Samudra) into `app/core/yoga_library.py` (29→44 detectors). Honest coverage (distinct yogas, exact match) **24.2% → 41.9%** (15→26/62). 46 yoga unit tests green; `house_judgment.py`/`synthesis_v2.py` byte-untouched (reading path, no strength-grade impact). Pinned by `test_yoga_coverage` |
| 37 | encode the Nabhāsa family + navāṁśa yogas | **landed (reading engine)** — 32 detectors (44→76): the whole Nabhāsa Ākṛti/Saṅkhyā/Dala block (Nos. 75–106, D1 occupancy via a `_occupied_arc` helper) + the 3 navāṁśa-dependent yogas (Gauri/Bharathi/Mridanga, reached through the D9 `compute_divisional_longitude` idiom) + solar (Ubhayachari/Ravi/Indra/Trilochana) + Matsya. Coverage **41.9% → 74.2%** (26→46/62). Six OCR-garbled corpus spellings mapped via the existing `_ALIAS` layer. `house_judgment.py`/`synthesis_v2.py` byte-untouched; per-path faithfulness pinned by `test_increment_37_nabhasa_and_navamsa_detectors`. Detection-completeness gain, not a strength claim (Raman: Ākṛti yogas "give clues as regards means of livelihood"). Residual 16 = intricate muladdhana/dhana + OCR-garbled tail |
| 38 | finish the definable yoga tail | **landed (reading engine)** — 12 more lord-definable yogas (76→88): the muladdhana dhana set (Bahudravyarjana, Matru/Putra/Satru-muladdhana, Balya Dhana), the Viparīta trio by name (Vimala/Harsha/Sarala), Sareera Soukhya, Sada Sanchara, Jaya, Pushkala. Coverage **74.2% → 90.3%** (46→56/62). "Vaiśeṣikāṁśa" clauses use the `_is_strong` proxy (documented under-fire). `house_judgment.py`/`synthesis_v2.py` byte-untouched; pinned by `test_increment_38_definable_tail_detectors`. Residual 6 honestly unencoded: Mahabhagya (needs gender+day/night), Vanchanachorabheethi (needs Gulika, not on `Chart`), and 4 OCR-garbled names with no recoverable definition — the practical corpus ceiling |
